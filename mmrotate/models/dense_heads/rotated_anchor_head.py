...
class RotatedAnchorHead(BaseDenseHead):
    ...
    def _get_targets_single(self,
        ...
        for i, _ in enumerate(inside_flags):
            if not inside_flags[i].any():
                return (None,) * 7
            # assign gt and sample anchors
            anchors.append(flat_anchors[i][inside_flags[i], :])
            flags.append(valid_flags[i][inside_flags[i]])
        ...
        if self.assign_by_circumhbbox is not None:
            ...
            assign_result = self.assigner.assign(
                anchors, flags, inside_flags, gt_bboxes,
                None if self.sampling else gt_labels, self.anchor_generator, img_meta)

        anchors, flat_anchors, inside_flags = torch.cat(anchors), torch.cat(flat_anchors), torch.cat(inside_flags)
    ...
    def get_bboxes(self,
        ...
        for img_id, _ in enumerate(img_metas):
            ...
            if with_nms:
                ...
                proposals = self._get_bboxes_single(cls_score_list, bbox_pred_list, mlvl_anchors, img_shape, scale_factor, cfg, rescale, with_nms)
